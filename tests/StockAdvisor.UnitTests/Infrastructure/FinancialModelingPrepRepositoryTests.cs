...
namespace StockAdvisor.UnitTests.Infrastructure
{
    public class FinancialModelingPrepRepositoryTests
    {
        ...
        [Fact]
        public async Task get_historical_async_checks_if_cache_contains_proper_data()
        {
            //Given
            var cacheResult = new List<CompanyPrice>();
            ...
            //When
            var callResult = await sut.GetHistoricalAsync(companySymbol);

            //Then
            MockMemoryCacheService.GetMethodWasCalledOnce(memoryCacheMock);
            callResult.Should().BeSameAs(cacheResult);
        }

        [Fact]
        public async Task get_historical_async_calls_httpclient_if_cache_get_returns_null()
        {
            //Given
            var expectedUri = new Uri("https://financialmodelingprep.com/api/v3/historical-price-full/AAPL?serietype=line");
            ...
            //When
            var result = await sut.GetHistoricalAsync(companySymbol);
        ...
        [Fact]
        public async Task get_historical_async_saves_http_request_result_to_cache()
        {
            //Given
            string companySymbol = "AAPL";
            ...
            //When
            await sut.GetHistoricalAsync(companySymbol);
        ...
        [Fact]
        public async Task get_historical_async_does_not_call_httpclient_if_cache_get_returns_not_null()
        {
            //Given
            string companySymbol = "AAPL";
            var cacheResult = new List<CompanyPrice>();
